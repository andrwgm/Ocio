...
export const ModificarCliente = () => {
    ...
    const [modificarError, setModificarError] = useState(false);
    ...
    const enviarRegistro = (event) => {
        ...
        try {
            ...
            if (!!username && !!password && !!nombre && !!apellidos && !!dni && !!fechanacimiento) {
                ...
                if (validarNombre(nombre) && validarApellidos(apellidos) && validarEmail(username) && validarContrasena(password) && validarDNI(dni) && validarFecha(fechanacimiento)) {
                    ...
                    axios({
                        ...
                        if (response.data.status === 'OK') {
                            setSuccess(true);
                            localStorage.setItem('perfilModif', 'OK');
                            navigate('/dashboard/cliente/profile');
                        } else {
                            navigate(0);
                        }
                    }).catch(error => {
                        setModificarError(true);
                    })
                }
            }
